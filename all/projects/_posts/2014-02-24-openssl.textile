...
The "RSA algorithm":http://en.wikipedia.org/wiki/RSA_(cryptosystem) is choose for this utility because there are "serious doubts":http://www.theguardian.com/world/2013/sep/05/nsa-how-to-remain-secure-surveillance in the validity of elliptic curve cryptography (given all those recent revelations)

However RSA has problems of its own: there are known side-channel attacks; if the computer that is encrypting the message has been compromised then you can break RSA encryption if you can measure fluctuations of the power consumption while encrypting/decrypting. 
...
This article does not do this kind of chaining.
