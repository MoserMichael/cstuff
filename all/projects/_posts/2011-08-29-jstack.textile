...
The access permissions say that only the user account that created the java process can access the named pipe, so be it.
...
h3. Script that monitors a given process.
...
All this information is written to standard output.
