...
The access permissions says that only one account - the user account that created the java process, can access the named pipe. So be it.
...
after the first option come additional options passed as is to jstack (optional) </pre>


h3. Script that monitors a given java process.
...
The script runs jstack in a loop with a period of <delay time> (default value 10 secoonds). 
