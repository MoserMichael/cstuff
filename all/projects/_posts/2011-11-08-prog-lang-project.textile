...
(GR1) This section defines the design goals / requirements for the Pooh programming language.

(GR2) Language must serve an educational purpose / be suited for the purpose of introducing kids to programming; It is not a functional programming language; one has assignments. It is not object oriented (no class concept with inheritance and virtual functions), instead it will have closures.
...
(R1) Language must be _somewhat_ dynamically typed; A variable is defined 
by assigning a value to a name; if identifier on the left hand side is not defined, it is hereby defined as a variable.
If the assignment is in a function, and variable is not yet defined, then the variable scope is in the function; if the assignment is a global scope, then the variable will be a global variable.
 
(R2) We can't do eval or create any bindings other than by declaring that binding explicitly in 
the code. Also variable binding is created either by assignment or by parameter 
declaration. 
...
(R15) String literals; there are many possible choices here "this document":http://en.wikipedia.org/wiki/String_literal explains the many notations for a string literal  ;
 
One question is why are there separate notations for strings that span one line and string that span multiple lines? One explanation is that strings that span multiple lines were introduced later as an added construct; I think it would be nice to have the same notation for both types of string.

One choice is to have The traditional delimiter for " as delimiter for start of multiple line string; and to adapt the PERL qq$aaa$ notation for different choice of delimiter character; 

Also very useful if not essential feature is to have "Variable interpolation":http://en.wikipedia.org/wiki/Variable_interpolation#Interpolation ; Of course it is limiting to only quote single variables, so Ruby can insert expressions into a string ; the Perl template system "Text::Template":http://search.cpan.org/~mjd/Text-Template-1.45/lib/Text/Template.pm has the interesting notation of putting a block into a template string; that is very cute, now it also needs a way to change the notation of start-of-block, end-of-block delimiters; so lets try to put that into the syntax.

The general form of this syntax is qqq( <delimiter of start of code> , <delimiter of end of code>  ) <character that denotes the end of string >  
Example:
qqq(${,})# this is quoted string ${ a + b } no need for escape sequences at all #

So this string construct does not need any escape characters at all ! So it will not have them. (of course it is a headache to parse all this ;-)  
