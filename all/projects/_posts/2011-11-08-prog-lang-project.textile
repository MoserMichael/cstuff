...
* Language must be _somewhat_ dynamically typed; A variable is defined 
...
* We can't do eval or create any bindings other than by declaring that binding explicitly in 
...
* Variable scope: A variable first assigned at global scope is global, a name assigned in a function is of local scope; 

* Look up the value of a variables; there are several possible ways / dilemmas.
...
* To be handy we have a built in regular expressions  (syntax like perl) or multi line strings 
(syntax like in python).  Also one needs array slices in order to return multiple variables from functions.
