...
Things lacking in old Basic dialects that where added in more powerfull scripting languages are variable scope, hashes, dynamic arrays, references, objects, regular expressions, closures. The designeers of Basic probably thought that it is essential to cut all these features out, in order to have a language that is easy to teach; I think it is still a challenge is to create a language that combines these goodies but that still remains easy to learn.
...
General purspose scripting languages have to cram in a lot of features; often design decisons are made with the purpose to simplify the runtime interpreter / execution envionment, these tradeoffs tend to be counter intuitive / hard to explain; Examples of such tradeoffs made by some languages are - function variables are by default global unless declared as of local scope; values can be used before having been assigned a value; 
