...
@#@ </notextile>  this construct is similar to HERE document; it says that the string #  is interpreted as the end of string delimiter; Unlike HERE document this delimiter can appear anywhere in the string, it is not limited to be at the start of a new line.

@##@aaa bbb ccc ## 
The end of string delimiter can a multi character string (without @ characters).
...
If the definition of a HERE document contains three strings then the first one is the end of string delimiter ( ## ) , the second one is the delimiter of start of embedded expression ( [[ ) ; the third one is the delimiter for end of embedded expression ( ]] ). 

One further rule is that the definition of a token has to be on the same line; <notextile> @ AAA @  </notextile> so AAA cannot include newline characters.
