...
The "previous article":http://mosermichael.github.com/cstuff/all/projects/2011/10/09/Non-Blocking-IO-with-coroutines.html describes a co-routine library integrated with the "libevent":http://libevent.org/      
 asynchronous event notification library, this article now presents WEBBY - an HTTP server that comes in two implementations, one that uses the co-routine library, and another one that handles each connection in a different operating system thread. WEBBY is an "embedded HTTP server":http://en.wikipedia.org/wiki/Embedded_HTTP_server , that means it should be easy to integrate the server with an existing application, and its main focus is on creating dynamic WEB content.
