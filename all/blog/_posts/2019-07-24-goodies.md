...
here [link to script](https://github.com/MoserMichael/myenv/blob/master/find-replace.sh) is a script that is supposed to take the pain out of this. Here is the help text
...
here are my vim customization [vim customizations & addons](https://github.com/MoserMichael/myenv/blob/master/VIMENV.md)
...
a scrip for [copying big files wih ssh](https://github.com/MoserMichael/myenv/blob/master/ssh-big.sh) ; Often the link via ssh is not very good, if you have to copy a big file then you migh have to tar/gz the file, ssh it to he other end, extract it and hen delete the temporary files, This script does that in one step.

a script that automates logging into a docker repository and that pushes a specified local image to the docker repository [link to script](https://github.com/MoserMichael/myenv/blob/master/kubernetes-utils/docker-push-repo.sh) by default the repository is quay.io

a script for debugging problems with pods: it describes the pod, extracts all containers from the pod spec and gets the logs for each container running inside the pod, along with the container spec [link to script](https://github.com/MoserMichael/myenv/blob/master/kubernetes-utils/pod-logs.sh)

a script for listing all deployments (globally or per namespace); for each deployment it displays information about its containers: name, image and command [link to script](https://github.com/MoserMichael/myenv/blob/master/kubernetes-utils/list-deployments-with-images.sh)

a script for scaling a deployment [scale deployment](https://github.com/MoserMichael/myenv/blob/master/kubernetes-utils/scale-deployment.sh), it waits until the actual number of pod replicas reaches the desired number of replicas, it fails if a given timeout has been reached.
