...
a script for debugging problems with pods: it describes the pod, extracts all containers from the pod spec and gets the logs for each container running inside the pod [link](https://github.com/MoserMichael/myenv/blob/master/kubernetes-utils/pod-logs.sh)
