...
s()
{
  find . -type f \( -name '*.cpp' -o '*.cxx' -o -name '*.hpp' -o '*.hxx' -o -name '*.h' \) -print0 2>/dev/null | xargs -0 grep $*
}
...
ctg()
{
  ...
  if [ "x$TOP_DIR" != "x" ]; then
      ...
      rm tags 2>/dev/null
      find . -type f \( -name '*.cpp' -o '*.cxx' -o -name '*.hpp' -o '*.hxx' -o -name '*.h' \) -print 0 | xargs -0 ctags -a --c++-kinds=+p --fields=+iaS --extra=+q --language-force=C++   
      popd >/dev/null
  else 
