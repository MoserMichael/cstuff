...
Luckily there is Scott Meyers who can explain the resulting mess of regulations; "Effective Modern C++" (purchase "here":http://shop.oreilly.com/product/0636920033707.do ) does a great job at explaining how all the new features of C++11 and c++14 legislation fit together.
...
* 'conservative' approach to software engineering (as Steve Yegge put it) in these quarters they don't like text based meta programming; however templates are considered to be powerful enough (somehow they don't mind the convoluted nature of C++ templates)
* in most shops they have coding guidelines that pick the desired C++ features and formulate 'best practices'; the stated aim here is to create a 'manageable subset' of the language. Well results are mixed (it's usually like this for codified instructions ...)


On the plus side of things c++11 and later standards address real pressing problems of C++98:

- the override specifier makes it hard to make errors while overriding virtual functions (Java had this  feature for a long time)
...
- auto keyword + type inference significantly reduces the verbosity of C++ code. In 1983 the auto keyword used to be a feature of c++, but it was later dropped !!! Later it got adopted by languages   like D and Java (since version 7); so the pressure was on C++ committee to make the language more 'powerful'.
- lambda functions are supposed to make stl::algorithms usable, but i doubt that these will be used: try to debug this code; i guess people will still prefer loops over iterators.
...
const auto cx = x;   // cx is const int  (same deduction as in: template<typename t> void f(T param))
const auto &rx = x;  // rx is const int (same deduction as in: template<typename t> void f(const T &param))
...
auto x = { 1 };	     // x is a std::initializer_list<int> and has an instance with value 27
auto x = { 1, 2 };	     // x is a std::initializer_list<int> and has an instance with values 1 and 2
auto x = { 1, 2, 3.0 };	     // error: values in initializer list must be of the same type.

C++14 allows function return types to be deduced with auto, but that does not work for this case !!! (why ???)


C++ function type can create generic lambda expression !!!! (but you can't instantiate with initializer list (?!))
...
C++11: decltype can be used in trailing return type specifications: allows to specify return type as returned type of decltype !!!
...
decltype(auto) f1()  // returns an int & (!!!oh my, this will crash when used!!!)
{
   int x = 2;
...
Widget w{ 3, 4.0 }; // Now the constructor with init list is called; AND IT DOES TYPE CONVERSION !@#@!
...
Widget w1(10, true); // calls con #1 (exact mach of parameter types)
Widget w3(10, 5.0);  // calls con #2 (exact mach of parameter types)
Widget w2{10, true}; // Can't call #3 - no implicit conversion to std::string. Call con #1 !!!@#@!!
Widget w4{10, 5.0};  // Can't call #3 - no implicit conversion to std::string. Call con #2 !!!@#@!!
Widget w5{};	     // calls default con; there is no empty std::initializer_list !!
Widget w5({});	     // forces creation of empty std::initializer_list !@#!\
Widget w5({});	     // same here.
...
class MyHack
{
   ...
   Vect imp_; // if you want to use the template this way you have to use typename.
};
...
int x = red; // values visible in the same scope as enum declaration ('namespace pollution'). Also cast from enum to int value.
	     // downside: more to type and name is longer (have to do scope::)
in c++11

enum class Color = { red, green };

Color x = Color::red; // now must qualify enum value with class name (no namespace pollution); also no implicit type conversion to integer not possible !! (but you can cast explicitly)
		      // C++98: comiler will choose the smallest type that can hold the num (based on maximum value); so type can change if maximum value changes !
		      // C++11: type to represent enum is always int. (allows for better forwarding of enum;

enum class Color : std::uint32_t; // C++11 also alows to change the representation type explicitly.
...
class Derived
{
  virtual foo(int a); // this will not compile; override says that foo name must override -> comply with the rules.
...
{
   return std::begin(container); // works because const C& returns const iterator !!! (used to work in c++98)
}
...
constexpr auto arraySize = 10; // arraySize is constant known during compilation

std::array<int, arraySize> data; // the size of an array must be known during compilation, so constexpr is fine
...
std::mutex a;
std::lock_guard<std::mutex> guard(m); //automatic lock in constructor, unlock in destructor.
...
class Widget {
  ...
  // problem: slicing problem - in case there are derived classes from std::string, passing derived class inst. will do implicit type conversion to string (and loose derived stuff)

  void setName(std::string newName) { name = std::move(newName); } 



v(item 18) emplace_insert in stl collections
...
{
  ~Widget();
  Widget(const Widget &&arg) = default; // tell it to do a default move constr
  Widget &operator=(Widget &&rhs) = default; // and move assignment despite specified destructor
...
- size of smart pointer with this deleter adds a second pointer data member (does delete to dispose of Foo when smart pointer obj. is destroyed)	
   ...
   std::unique<Foo, decltype(deleteF)> ptr( new Foo(), deleteF );// note that thisison
...
(item 21) std::shared_ptr - smart pointer with reference counting (aka shared resource management)
 ...
 - if you don't like this rule:
    // the base class always has pointer to the controll block; so it can be asked repeatedly to create smart ptr instance using the same controll block
...
- std::forward is a conditional cast to rvalue; 
  ...
  forward( f );			// called with lvalue; now std::forward does not cast; forwarding an lvalue creates copies each time that the argument is forwarded.
  forward( std::move(f) );	// called with rvalue: std::forward casts to rvalue only if forward was called with rvalue !!!! (this is done by clever type deduction and is explained later)
...
class Foo
{
	void add(T && arg ); // also rvalue reference: this is add of a specific template instance !
...
int aaa;
auto e = [aaa] (int c) { return c * val_; };   // must specify captured variables by name in []; copied by value; can only visible locals.


auto e = [=] (int c) { return c * val_; };   // copy by value (also ptrs copied by value); can access data member of enclosing class + locals in enclosing scope
 
auto e = [=] (int c) { return c * val_; };   // copy by ref; can access data member of enclosing class + locals in enclosing scope.
...
public:
  ...
  int doo_local(int arg)
  {
    ...
    //auto e = [] (int c) { return c * vall; }; // does not compile, must specify the imported variable
...
- c++11 no way to move anything into closur (either copy by val or copy by ref(
