...
With the aim of furthering the Singularity event - I would now like to prose a better test that should tell if a machine is intelligent or not.

Normally a program dies when its environment changes in some unexpected ways (this is also called "Software rot":http://en.wikipedia.org/wiki/Software_rot ); I would say that a program (or any other technical system) is intelligent, if it can adapt by itself to unexpected changes of its environment. This test would not require intervention of outside entities, unlike the "turing test":http://en.wikipedia.org/wiki/Turing_test  for example. I would call this intelligence test the 'Wall-E test' - name after robot "Wall-E":http://en.wikipedia.org/wiki/WALL-E who by means of intelligence and wit could adjust to a total wasteland.
