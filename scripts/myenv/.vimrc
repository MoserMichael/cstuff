...
"specify tag file
"set tags=/home/radware/prj/tags
...
"highlight search results (so matches are marked specially)
set hlsearch 
...
function! s:RunBuild()
    ...
    let builcmd=""
    if executable("./make")
	let buildcmd = "./make  2> " . tmpfile . " >/dev/null"
    else
...
if !exists("Grep_Default_Filelist")
    let Grep_Default_Filelist = '*.cc *.c *.cpp *.cxx *.h *.inl *.hpp *.hxx *.y *.l'
endif
