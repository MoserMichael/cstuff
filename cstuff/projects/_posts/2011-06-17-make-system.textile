...
* Ability to add compiler and linker options from environment to each target (environment variables EXTERNAL_CFLAGS , EXTERNAL_LDFLAGS ).
...
Also there are some nice built in tricks that are sometimes very useful during development
	make CPP_LISTING=1
...
Removes the build result directory; everything that is built is hereby removed.
