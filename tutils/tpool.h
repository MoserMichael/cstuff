...
/**
  ...
  Includes a callback that is invoked in order to process the request, and callback to destructor.

  somehow similar to java concept of java.lang.Runnable.

  @{ 
 */

/**
 * @brief calback function of working on a request
 * @param request - this pointer
 * @param thread_ctx - per worker thread context
 * @param pool_ctx - perl pool context
 */
typedef void  (*RUNNABLE_HANDLER) (struct tagRUNNABLE *request, void *thread_ctx, void *pool_ctx);
...
/**
 * @brief create per thread context
 * @param pool_ctx - per pool context
 */
typedef void *(*THREAD_INIT) (void *pool_ctx);

/**
 * @brief clean up of per thread context.
 * @param thread_ctx - this pointer, this function is supposed to clean it up
 * @param pool_ctx - per pool context
 */
typedef void  (*THREAD_FINISH) (void *thread_ctx, void *pool_ctx);
...
/**
 ...
 @param process_result  a callback that receives work request after invoking it's run method. Argument value can be NULL (in this case it is not invoked)
 @param thread_init a callback creates per thread data; Argument value can be NULL (in this case it is not invoked)
 @param thread_finish a callback cleans up per thread context; Argument value can be NULL (in this case it is not invoked)
